...
repl ::
  ...
  -> N.Name
  -> Task B.Builder
repl root details ansi (Build.ReplArtifacts home modules localizer annotations) name = do
  objects <- finalizeObjects =<< loadObjects root details modules
  let graph = objectsToGlobalGraph objects
  return $
...
lookupMain ::
     Pkg.Name
  -> Map.Map ModuleName.Raw Opt.LocalGraph
  -> Build.Root
...
loadTypes root ifaces modules =
  Task.eio id $ do
    ...
    let !foreigns =
          Extract.mergeMany
            (Map.elems (Map.mapWithKey Extract.fromDependencyInterface ifaces))
    results <- traverse readMVar mvars
    ...
    Build.Cached name _ ciMVar -> do
      ...
      case cachedInterface of
        Build.Unneeded -> do
          ...
          return mvar
        Build.Loaded iface -> newMVar (Just (Extract.fromInterface name iface))
        Build.Corrupted -> newMVar Nothing
