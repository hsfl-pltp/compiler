...
-- HELPERS
isKernel :: Name -> Bool
isKernel (Name author _) = author == elm || author == elm_explorations
...
toUrl :: Name -> String
toUrl (Name author project) = Utf8.toChars author ++ "/" ++ Utf8.toChars project

toFilePath :: Name -> FilePath
toFilePath (Name author project) = Utf8.toChars author </> Utf8.toChars project
...
toName author project = Name author (Utf8.fromChars project)

{-# NOINLINE dummyName #-}
dummyName :: Name
dummyName = toName (Utf8.fromChars "author") "project"

{-# NOINLINE kernel #-}
kernel :: Name
kernel = toName elm "kernel"
...
{-# NOINLINE html #-}
html :: Name
html = toName elm "html"

{-# NOINLINE json #-}
json :: Name
json = toName elm "json"

{-# NOINLINE http #-}
http :: Name
http = toName elm "http"
...
{-# NOINLINE webgl #-}
webgl :: Name
webgl = toName elm_explorations "webgl"

{-# NOINLINE linearAlgebra #-}
linearAlgebra :: Name
linearAlgebra = toName elm_explorations "linear-algebra"
...
elm_explorations = Utf8.fromChars "elm-explorations"
...
(==>) moduleName package = (Utf8.fromChars moduleName, package)
...
decoder = D.customString parser (,)

encode :: Name -> E.Value
encode name = E.chars (toChars name)

keyDecoder :: (Row -> Col -> x) -> D.KeyDecoder x Name
keyDecoder toError =
  let keyParser = P.specialize (\(r, c) _ _ -> toError r c) parser
...
parseName ::
     (Word8 -> Bool) -> (Word8 -> Bool) -> P.Parser (Row, Col) (Utf8.Utf8 t)
    ...
    if pos >= end
      then eerr row col (,)
            ...
            in if not (isGoodStart word)
                 then eerr row col (,)
                 else let (# isGood, newPos #) =
                            chompName isGoodInner (plusPtr pos 1) end False
...
isLower :: Word8 -> Bool
isLower word = 0x61 <= word && word <= 0x7A {-a-}
   {-z-}

isLowerOrDigit :: Word8 -> Bool
isLowerOrDigit word =
  0x61 <= word && word <= 0x7A || 0x30 <= word && word <= 0x39

isAlphaOrDigit :: Word8 -> Bool
isAlphaOrDigit word =
  0x61 <= word && word <= 0x7A ||
  0x41 <= word && word <= 0x5A || 0x30 <= word && word <= 0x39

chompName ::
     (Word8 -> Bool) -> Ptr Word8 -> Ptr Word8 -> Bool -> (# Bool, Ptr Word8 #)
