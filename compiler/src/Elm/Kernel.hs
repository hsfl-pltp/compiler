{-# OPTIONS_GHC -Wall #-}
{-# LANGUAGE BangPatterns, EmptyDataDecls, OverloadedStrings,
  UnboxedTuples #-}
...
countFields chunks = foldr addField Map.empty chunks
...
type Foreigns = Map.Map ModuleName.Raw Pkg.Name
...
chompChunks ::
  ...
  -> Ptr Word8
  -> [Chunk]
  -> (# [Chunk], Ptr Word8, Row, Col #)
chompChunks vs es fs src pos end row col lastPos revChunks =
  if pos >= end
    then let !js = toByteString src lastPos end
          in (# reverse (JS js : revChunks), pos, row, col #)
    else let !word = unsafeIndex pos
          in if word == 0x5F {-_-}
               then let !pos1 = plusPtr pos 1
                        !pos3 = plusPtr pos 3
                     in if pos3 <= end && unsafeIndex pos1 == 0x5F {-_-}
                          then let !js = toByteString src lastPos pos
                                in chompTag
                                     vs
                                     es
                                     fs
                      ...
                      then chompChunks
                             ...
                             lastPos
                             revChunks
                      else let !newPos = plusPtr pos (getCharWidth word)
                            in chompChunks
                                 vs
                                 es
                                 fs
...
chompTag vs es fs src pos end row col revChunks =
  let (# newPos, newCol #) = Var.chompInnerChars pos end col
   ...
   in if word == 0x24 {-$-}
        then let !name = Name.fromPtr pos newPos
              in chompChunks vs es fs src newPos end row newCol newPos $
                 ElmField name : revChunks
        else let !name = Name.fromPtr tagPos newPos
              in if 0x30 <= word && word <= 0x39 {-0-}
                     {-9-}
                   then let (enum, newEnums) = lookupEnum (word - 0x30) name es
                         in chompChunks
                              vs
                              newEnums
                              fs
                   ...
                   else if 0x61 <= word && word <= 0x7A {-a-}
                            {-z-}
                          then let (field, newFields) = lookupField name fs
                                in chompChunks
                                     vs
                                     es
                                     newFields
...
-- FIELDS
type Fields = Map.Map Name.Name Int
...
-- ENUMS
type Enums = Map.Map Word8 (Map.Map Name.Name Int)

lookupEnum :: Word8 -> Name.Name -> Enums -> (Int, Enums)
lookupEnum word var allEnums =
  let enums = Map.findWithDefault Map.empty word allEnums
...
-- PROCESS IMPORTS
type VarTable = Map.Map Name.Name Chunk
...
addImport pkg foreigns vtable (Src.Import (A.At _ importName) maybeAlias exposing) =
  if Name.isKernel importName
    then case maybeAlias of
           Just _ ->
             error
               ("cannot use `as` with kernel import of: " ++
                Name.toChars importName)
           Nothing ->
             let home = Name.getKernel importName
                 add table name =
                   Map.insert
                     (Name.sepBy 0x5F home name)
                     (JsVar home name)
                     table {-_-}
              in List.foldl' add vtable (toNames exposing)
    else let home =
               ModuleName.Canonical
                 (Map.findWithDefault pkg importName foreigns)
                 importName
             prefix = toPrefix importName maybeAlias
             add table name =
               Map.insert (Name.sepBy 0x5F prefix name) (ElmVar home name) table {-_-}
          in List.foldl' add vtable (toNames exposing)
      ...
      if Name.hasDot home
        then error
               ("kernel imports with dots need an alias: " ++
                show (Name.toChars home))
        else home

toNames :: Src.Exposing -> [Name.Name]
toNames exposing =
  case exposing of
    Src.Open -> error "cannot have `exposing (..)` in kernel code."
    Src.Explicit exposedList -> map toName exposedList

toName :: Src.Exposed -> Name.Name
toName exposed =
  case exposed of
    Src.Lower (A.At _ name) -> name
    Src.Upper (A.At _ name) Src.Private -> name
    Src.Upper _ (Src.Public _) ->
      error "cannot have Maybe(..) syntax in kernel code header"
    Src.Operator _ _ -> error "cannot use binops in kernel code"
