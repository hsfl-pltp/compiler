...
generateForRepl ansi localizer (Opt.GlobalGraph graph _) home name (Can.Forall _ tipe) =
  let mode = Mode.Dev Nothing
   ...
   in "process.on('uncaughtException', function(err) { process.stderr.write(err.toString() + '\\n'); process.exit(1); });" <>
      Functions.functions <>
      stateToBuilder evalState <> print ansi localizer home name tipe
...
generateForReplEndpoint localizer (Opt.GlobalGraph graph _) home maybeName (Can.Forall _ tipe) =
  let name = maybe Name.replValueToPrint id maybeName
   ...
   in Functions.functions <>
      stateToBuilder evalState <> postMessage localizer home maybeName tipe
  ...
  let addDeps deps someState = Set.foldl' (addGlobal mode graph) someState deps
   in case graph ! global of
        Opt.Define expr deps ->
          addStmt (addDeps deps state) (var global (Expr.generate mode expr))
        ...
        Opt.Ctor index arity ->
          addStmt state (var global (Expr.generateCtor mode global index arity))
        Opt.Link linkedGlobal -> addGlobal mode graph state linkedGlobal
...
addStmt state stmt = addBuilder state (JS.stmtToBuilder stmt)
...
generateCycle mode (Opt.Global home _) names values functions =
  JS.Block
    ...
    , case map (generateRealCycle home) values of
        ...
        realBlock@(_:_) ->
          case mode of
            Mode.Prod _ -> JS.Block realBlock
            Mode.Dev _ ->
  ...
  case def of
    Opt.Def name expr ->
      JS.Var
        ...
        (Expr.codeToExpr (Expr.generate mode expr))
    Opt.TailDef name args expr ->
      JS.Var
        (JsName.fromGlobal home name)
        (Expr.codeToExpr (Expr.generateTailDef mode name args expr))

generateSafeCycle ::
     Mode.Mode -> ModuleName.Canonical -> (Name.Name, Opt.Expr) -> JS.Stmt
...
-- GENERATE MANAGER
generateManager ::
     Mode.Mode -> Graph -> Opt.Global -> Opt.EffectsType -> State -> State
generateManager mode graph (Opt.Global home@(ModuleName.Canonical _ moduleName) _) effectsType state =
  let managerLVar =
      ...
      (deps, args, stmts) = generateManagerHelp home effectsType
      createManager =
        JS.ExprStmt $
...
{-# NOINLINE leaf #-}
leaf :: JS.Expr
leaf = JS.Ref (JsName.fromKernel Name.platform "leaf")

generateManagerHelp ::
     ModuleName.Canonical
  -> Opt.EffectsType
  -> ([Opt.Global], [JS.Expr], [JS.Stmt])
  ...
  let dep name = Opt.Global home name
   ...
   in case effectsType of
        ...
        Opt.Fx ->
          ...
          , [generateLeaf home "command", generateLeaf home "subscription"])
...
generateExports mode (Trie maybeMain subs) =
  let starter end =
        case maybeMain of
          ...
          Just (home, main) ->
            "{'init':" <>
            JS.exprToBuilder (Expr.generateMain mode home main) <> end
