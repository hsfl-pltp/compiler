...
-- OPTIMIZE
type Result i w a = Result.Result i w E.Error a

type Annotations = Map.Map Name.Name Can.Annotation
...
optimize annotations (Can.Module home _ _ decls unions aliases _ effects) =
  ...
  addAliases home aliases $ Opt.LocalGraph Nothing Map.empty Map.empty

-- UNION
type Nodes = Map.Map Opt.Global Opt.Node
...
addCtorNode ::
     ModuleName.Canonical -> Can.CtorOpts -> Nodes -> Can.Ctor -> Nodes
...
addAliases ::
  ...
  -> Opt.LocalGraph
  -> Opt.LocalGraph
addAliases home aliases graph = Map.foldrWithKey (addAlias home) graph aliases
  ...
  case tipe of
    Can.TRecord fields Nothing ->
      let function =
            ...
            Opt.Record $ Map.mapWithKey (\field _ -> Opt.VarLocal field) fields
          node = Opt.Define function Set.empty
       in Opt.LocalGraph
            main
...
addRecordCtorField ::
  ...
  -> Map.Map Name.Name Int
addRecordCtorField name _ fields = Map.insertWith (+) name 1 fields

-- ADD EFFECTS
addEffects ::
     ModuleName.Canonical -> Can.Effects -> Opt.LocalGraph -> Opt.LocalGraph
addEffects home effects graph@(Opt.LocalGraph main nodes fields) =
  case effects of
    Can.NoEffects -> graph
    Can.Ports ports -> Map.foldrWithKey (addPort home) graph ports
    Can.Manager _ _ _ manager ->
      let fx = Opt.Global home "$fx$"
          ...
          newNodes =
            case manager of
              Can.Cmd _ ->
                Map.insert cmd link $ Map.insert fx (Opt.Manager Opt.Cmd) nodes
              Can.Sub _ ->
                Map.insert sub link $ Map.insert fx (Opt.Manager Opt.Sub) nodes
              Can.Fx _ _ ->
                Map.insert cmd link $
                Map.insert sub link $ Map.insert fx (Opt.Manager Opt.Fx) nodes
...
addDecls home annotations decls graph =
  case decls of
    ...
    Can.SaveTheEnvironment -> Result.ok graph
...
addDef home annotations def graph =
  case def of
    Can.Def (A.At region name) args body -> do
...
addDefHelp region annotations home name args body graph@(Opt.LocalGraph _ nodes fieldCounts) =
  if name /= Name._main
    then Result.ok (addDefNode home name args body Set.empty graph)
    else let (Can.Forall _ tipe) = annotations ! name
          ...
          in case Type.deepDealias tipe of
               ...
               _ -> Result.throw (E.BadType region tipe)
...
addDefNode home name args body mainDeps graph =
  let (deps, fields, def) =
   ...
   in addToGraph
        ...
        (Opt.Define def (Set.union deps mainDeps))
        fields
        graph
...
addRecDefs ::
     ModuleName.Canonical -> [Can.Def] -> Opt.LocalGraph -> Opt.LocalGraph
addRecDefs home defs (Opt.LocalGraph main nodes fieldCounts) =
  let names = reverse (map toName defs)
      ...
      (deps, fields, State values funcs) =
        Names.run $ foldM (addRecDef cycle) (State [] []) defs
   in Opt.LocalGraph
        main
        (Map.insert
           cycleName
           (Opt.Cycle names values funcs deps)
           (Map.union links nodes))
        (Map.unionWith (+) fields fieldCounts)
...
addLink home link def links =
  case def of
    Can.Def (A.At _ name) _ _ -> Map.insert (Opt.Global home name) link links
...
addRecDef cycle state def =
  case def of
    Can.Def (A.At _ name) args body -> addRecDefHelp cycle state name args body
