...
compile rawBranches =
  let format (pattern, index) = Branch index [(Empty, pattern)]
...
dearg (Can.PatternCtorArg _ _ pattern) = pattern
...
testsAtPath selectedPath branches =
  let allTests = Maybe.mapMaybe (testAtPath selectedPath) branches
      skipVisited test curr@(uniqueTests, visitedTests) =
        if Set.member test visitedTests
          then curr
          else (test : uniqueTests, Set.insert test visitedTests)
  ...
  case List.lookup selectedPath pathPatterns of
    ...
    Just (A.At _ pattern) ->
      case pattern of
        ...
        Can.PInt int -> Just (IsInt int)
        Can.PStr str -> Just (IsStr str)
        Can.PChr chr -> Just (IsChr chr)
        Can.PBool _ bool -> Just (IsBool bool)
...
edgesFor path branches test =
  (test, Maybe.mapMaybe (toRelevantBranch test path) branches)
    ...
    Found start (A.At region pattern) end ->
      case pattern of
            ...
            IsCtor _ testName _ _ _
              | name == testName ->
                ...
                case map dearg ctorArgs of
                  [arg]
                    | numAlts == 1 -> start ++ [(Unbox path, arg)] ++ end
                  args -> start ++ subPositions path args ++ end
            _ -> Nothing
        Can.PList [] ->
          case test of
            IsNil -> Just (Branch goal (start ++ end))
            _     -> Nothing
        Can.PList (hd:tl) ->
          case test of
            IsCons ->
              let tl' = A.At region (Can.PList tl)
               in Just
                    (Branch goal (start ++ subPositions path [hd, tl'] ++ end))
            _ -> Nothing
        Can.PCons hd tl ->
          case test of
            IsCons ->
        ...
        Can.PChr chr ->
          case test of
            IsChr testChr
              | chr == testChr -> Just (Branch goal (start ++ end))
            _ -> Nothing
        Can.PStr str ->
          case test of
            IsStr testStr
              | str == testStr -> Just (Branch goal (start ++ end))
            _ -> Nothing
        Can.PInt int ->
          case test of
            IsInt testInt
              | int == testInt -> Just (Branch goal (start ++ end))
            _ -> Nothing
        Can.PBool _ bool ->
          case test of
            IsBool testBool
              | bool == testBool -> Just (Branch goal (start ++ end))
            _ -> Nothing
...
isIrrelevantTo selectedPath (Branch _ pathPatterns) =
  case List.lookup selectedPath pathPatterns of
    Nothing      -> True
    Just pattern -> not (needsTests pattern)
...
pickPath branches =
  let allPaths = Maybe.mapMaybe isChoicePath (concatMap _patterns branches)
...
addWeights toWeight paths = map (\path -> (path, toWeight path)) paths
...
smallDefaults branches path = length (filter (isIrrelevantTo path) branches)

smallBranchingFactor :: [Branch] -> Path -> Int
smallBranchingFactor branches path =
  let (edges, fallback) = gatherEdges branches path
   in length edges +
      (if null fallback
         then 0
         else 1)
...
instance Binary Test where
  put test =
    case test of
      IsCtor a b c d e ->
        putWord8 0 >> put a >> put b >> put c >> put d >> put e
      IsCons -> putWord8 1
      IsNil -> putWord8 2
      IsTuple -> putWord8 3
