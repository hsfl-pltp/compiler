...
import           GHC.Exts                         (ByteArray#, Char (C#),
                                                   Int (I#), MutableByteArray#,
                                                   Ptr (Ptr), RealWorld,
                                                   copyAddrToByteArray#,
                                                   copyByteArray#,
                                                   copyByteArrayToAddr#,
...
import           Prelude                          hiding (String, all, any,
                                                   concat)
...
{-# NOINLINE empty #-}
empty :: Utf8 t
empty = runST (freeze =<< newByteArray 0)

isEmpty :: Utf8 t -> Bool
isEmpty (Utf8 ba#) = isTrue# (sizeofByteArray# ba# ==# 0#)

-- SIZE
size :: Utf8 t -> Int
size (Utf8 ba#) = I# (sizeofByteArray# ba#)
...
containsHelp word# ba# !offset# len# =
  if isTrue# (offset# <# len#)
    then if isTrue# (eqWord# word# (indexWord8Array# ba# offset#))
           then True
...
startsWithChar isGood bytes@(Utf8 ba#) =
  if isEmpty bytes
    then False
             ...
             !char
               | isTrue# (ltWord# w# 192##) = C# (chr# (word2Int# w#))
               | isTrue# (ltWord# w# 224##) = chr2 ba# 0# w#
               | isTrue# (ltWord# w# 240##) = chr3 ba# 0# w#
               | True = chr4 ba# 0# w#
          in isGood char
...
findDividers divider# ba# !offset# len# revOffsets =
  if isTrue# (offset# <# len#)
    then findDividers divider# ba# (offset# +# 1#) len# $
    ...
    char:chars
      ...
      | n < 0x10000 -> do
        ...
        writeWord8
          mba
          (offset + 1)
          (fromIntegral ((shiftR n 6 .&. 0x3F) + 0x80))
      ...
      | otherwise -> do
        ...
        writeWord8
          mba
          (offset + 1)
          (fromIntegral ((shiftR n 12 .&. 0x3F) + 0x80))
        writeWord8
          mba
          (offset + 2)
          (fromIntegral ((shiftR n 6 .&. 0x3F) + 0x80))
...
-- TO CHARS
toChars :: Utf8 t -> [Char]
toChars (Utf8 ba#) = toCharsHelp ba# 0# (sizeofByteArray# ba#)

toCharsHelp :: ByteArray# -> Int# -> Int# -> [Char]
toCharsHelp ba# offset# len# =
  if isTrue# (offset# >=# len#)
    then []
    else let !w# = indexWord8Array# ba# offset#
             !(# char, width# #)
               | isTrue# (ltWord# w# 192##) = (# C# (chr# (word2Int# w#)), 1# #)
               | isTrue# (ltWord# w# 224##) = (# chr2 ba# offset# w#, 2# #)
               | isTrue# (ltWord# w# 240##) = (# chr3 ba# offset# w#, 3# #)
...
chr3 ba# offset# firstWord# =
  let !i1# = word2Int# firstWord#
      ...
      !c1# = uncheckedIShiftL# (i1# -# 224#) 12#
      !c2# = uncheckedIShiftL# (i2# -# 128#) 6#
      !c3# = i3# -# 128#
   in C# (chr# (c1# +# c2# +# c3#))
...
chr4 ba# offset# firstWord# =
  let !i1# = word2Int# firstWord#
      ...
      !c1# = uncheckedIShiftL# (i1# -# 240#) 18#
      !c2# = uncheckedIShiftL# (i2# -# 128#) 12#
      !c3# = uncheckedIShiftL# (i3# -# 128#) 6#
      !c4# = i4# -# 128#
...
toBuilderHelp !bytes@(Utf8 ba#) k = go 0 (I# (sizeofByteArray# ba#))
  where
...
toEscapedBuilderHelp ::
     Word8 -> Word8 -> Utf8 t -> B.BuildStep a -> B.BuildStep a
...
escape before@(W8# before#) after ptr name@(Utf8 ba#) offset@(I# offset#) len@(I# len#) i@(I# i#) =
  if isTrue# (i# <# len#)
    then if isTrue# (eqWord# before# (indexWord8Array# ba# (offset# +# i#)))
           then do
...
copyFromByteString len (B.PS fptr offset _) =
  unsafeDupablePerformIO
    (do mba <- stToIO (newByteArray len)
        stToIO
          (copyFromPtr (unsafeForeignPtrToPtr fptr `plusPtr` offset) mba 0 len)
