...
-- HAS DOT
hasDot :: Name -> Bool
hasDot name = Utf8.contains 0x2E name {- . -}

splitDots :: Name -> [Name]
splitDots name = Utf8.split 0x2E name {- . -}
...
writeDigitsAtEnd !mba !oldOffset !n = do
...
fromTypeVariable name@(Utf8.Utf8 ba#) index =
  if index <= 0
    then name
    else let len# = sizeofByteArray# ba#
             end# = indexWord8Array# ba# (len# -# 1#)
          in if isTrue# (leWord# 48## end#) && isTrue# (leWord# end# 57##)
               then runST
       ...
       in runST
            (ST $ \s ->
               case newByteArray# (len# +# 3#) s of
                 (# s, mba# #) ->
                   case writeWord8Array# mba# 0# 95## s {-_-}
                         of
                     s ->
                       case writeWord8Array# mba# 1# 77## s {-M-}
                             of
                         s ->
                           case writeWord8Array# mba# 2# 36## s {-$-}
                                 of
                             s ->
...
{-# NOINLINE blank #-}
blank :: Name
blank = fromWords [0x5F, 0x4D, 0x24] {-_M$-}
