...
run paths flags@(Flags _ _ _ report _) = do
...
buildPaths style root details paths =
  Task.eio Exit.MakeCannotBuild $ Build.fromPaths style root details paths
...
isMain targetName modul =
  case modul of
    ...
    Build.Cached name mainIsDefined _ -> mainIsDefined && name == targetName
...
toBuilderArduino root details desiredMode artifacts =
  Task.mapError Exit.MakeBadGenerate $
  case desiredMode of
    Dev -> Generate.devArduino root details artifacts
...
hasExt ext path = FP.takeExtension path == ext && length path > length ext

isDevNull :: String -> Bool
isDevNull name = name == "/dev/null" || name == "NUL" || name == "$null"
